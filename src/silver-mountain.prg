...
DATA 31SAFE IN OGBANS CHAMBER,31CUPBOARD IN A CORNER
DATA 11NARROW PASSAGE,16CAVE
DATA 11WOODMANS HUT,42SIDE OF A WOODED VALLEY
...
DATA KETCH,BRICKS,WINDMILL,SACKS,OGBANS BOAR,WHEEL
DATA PONY,GRAVESTONES,POOL,GATES,HANDLE,HUT,VINE,INSCRIPTING,TROLL,RUBBLE
...
DATA MISTY POOL,WELLBOTTOM,AOLD KILN,MOUNTAIN HUT
...
DATA 80,70,60,69,4,72,63,52,20,11,1,14,36,54,61,21,32,10,50
DATA 29,59,34,13,80,30,81,47,74
DATA 1,2,3,4,5,9,12,13,16,17,20,21,22
LET Z$="":FOR I= TO LEN(R$)
LET C$=MID$(R$,I,1):IF C$<"A" THEN LET Z$=Z$+C$:GOTO 4300
LET C=ASC(C$)-1:IF C=64 THEN LET C=90
LET Z$=Z$+CHR$(C)
NEXT I:LET R$=Z$:RETURN
LET J$="SSSSSSSS":LET NG=0
LET MP=D/2:GOSUB 4400
PRINT "YOU ARE LOST IN THE":PRINT "     TUNNELS"
PRINT "WHICH WAY? (N,S,W OR E)"
IF NG>15 THEN PRINT "(OR G TO GIVE UP!)"
PRINT:INPUT W$:LET J$=RIGHT$(J$+RIGHT$(W$,1),8)
IF W$="G" THEN LET F(56)=1:RETURN
IF J$<>G$(MP) THEN LET NG=NG+1:GOTO 4320
RETURN
CLS:PRINT
...
PRINT "=========================================="
PRINT:PRINT:RETURN
FOR I=1 TO 80:READ E$(I):NEXT I
FOR I=1 TO G:READ C(I):NEXT I
FOR I=1 TO 13:READ A :LET F(A)=1:NEXT I
LET F(41)=INT(RND(1)*900)+100:LET F(42)=INT(RND(1)*3)+2
LET F(44)=4:LET F(57)=68:LET F(58)=54:LET F(59)=15:LET F(52)=INT(RND(1)*3)
LET R=77:LET R$="GOOD LUCK ON YOUR QUEST!"
LET G$(1)="":FOR I=1 TO 8
KET F$=MID$(B$,1+INT(RND(1)*4)*3,1)
LET G$(1)=G$(1)+F$
IF F$="N" THEN LET L$="S"
IF F$="S" THEN LET L$="N"
IF F$="E" THEN LET L$="W"
...
LET G$(2)=L$+G$(2)
NEXT I:RETURN
GOSUB 4640:GOSUB 4670
LET R=F(69):LET R$="OK, CARRY ON"
RETURN
LET F(69)=R:GOSUB 4640:GOSUB 4760:PRINT "BYE...":STOP
PRINT:PRINT "PLEASE ENTER FILE NAME":INPUT FL$
RETURN
REM READ DATA FILE
REM
PRINT "OK. SEARCHING FOR ";FL$
X=OPENIN(FL$):PRINT "OK. LOADING"
FOR I=1 TO 80:INPUT#X,E$(I):NEXT
FOR I=1 TO G:INPUT#X,C(I):NEXT
OFR I=1 TO 70:INPUT#X,F(I):NEXT
INPUT#X,G$(1):INPUT#X,G$(2)
CLOSE#X:RETURN
REM SAVE DATA FILE
REM
X=OPENOUT(FL$):PRINT "OK. SAVING"
FOR I=1 TO 80:PRINT#X,E$(I):NEXT
...
LET LS=1:LET LP=1
FOR I=1 TO LEN(J$)
IFMID$(J$,I,1)=" "ANDLL>EL THEN PRINT MID$(J$,LP,LS-LP):LET LL=I-LS:LET LP=LS+1
IF MID$(J$,I,1)=" " THEN LET LS=I
LET LL=LL+1:NEXT I
PRINT MID$(J$,LP,LEN(J$)-LP);
RETURN
